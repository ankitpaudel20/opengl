...
public:
	...
	void Draw(Shader& shader, const glm::mat4& rot=glm::mat4(1.0), const glm::mat4& trans = glm::mat4(1.0), const glm::mat4& scale = glm::mat4(1.0))
	{
		for (auto& i : meshes) {
			i.rotation = rot;
			i.translation = trans;
	...
	void setTranslation(const glm::mat4& rot) {
		for (auto& i : meshes)
			i.translation = rot;
	}

	void setScaling(const glm::mat4& rot) {
		for (auto& i : meshes)
			i.scaling = rot;
	}
...
private:
	...
	void processNode(aiNode* node, const aiScene* scene, int num = 0)
	{
		...
		{
			processNode(node->mChildren[i], scene,++num);
		}
